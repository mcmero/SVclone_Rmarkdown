...
```{r Figure2b, fig.height=20, fig.width=10}
...
```{r Figure2c, fig.height=5, fig.width=6}
cm <- melt(ccf_met)
...
```{r Figure2d, fig.height=5, fig.width=7}
...
ggrocs(rocs) + theme(axis.text = element_text(size = tsize),
                     ...
                     axis.title = element_blank()) + geom_step(size=1) +
...
```{r SuppTable3}
# add 001bM and gM data
bm <- read.delim('001_svclone_results/001bM/001bM_filtered_svs.tsv', sep='\t')
gm <- read.delim('001_svclone_results/001gM/001gM_filtered_svs.tsv', sep='\t')

hetCN <- data.table(mix='001bM', heterogenousCN=bm$gtype1 != bm$gtype2)
hetCN <- rbind(hetCN,
               data.table(mix='001gM', heterogenousCN=gm$gtype1 != gm$gtype2))
hetCN <- rbind(hetCN,
               data.table(mix=sv_data$mix, heterogenousCN=sv_data$gtype1 != sv_data$gtype2))
hetCN <- hetCN[, sum(heterogenousCN)/length(heterogenousCN), by=c('mix')]
hetCN$V1 <- round(hetCN$V1, 2)
colnames(hetCN)[2] <- 'heterogenous_CN_percent'
...
```{r calc_se_metrics}
source('R/CN_perturbation_experiments.R')

se_basedir <- '001_svclone_results/001_mixes_single_end/'
# uncomment below to generate data
# SVclone/cluster_with_ccube.R will then
# need to be run on output
# generate_se_svs(basedir, se_basedir)

svse_data <- NULL
for(i in seq(0.1, 0.9, 0.1)) {
    mix <- paste(i, 1-i, sep='-')
    basename <- paste('001bM_p', i, '_001gM_p', 1-i, sep='')
    for(side in c('side1', 'side2')) {
        ccubeResultRDataFile = paste0(se_basedir, basename,
                                      paste('/ccube', side, 'out/', sep='_'),
                                      'ccube_out/snvs/', basename, '_ccube_snv_results.RData')
        load(ccubeResultRDataFile)

        svs <- data.frame(data='snvs', side=side, snvRes$ssm, 
                          sv=snvRes$ssm$mutation_id, mix=mix, p1=i, p2=1-i)

        svin <- read.delim(paste0(resultFolder, basename, '/', basename, '_filtered_svs.tsv'))
        svin$mutation_id <- paste(svin$chr1, svin$pos1, svin$dir1, svin$chr2, svin$pos2, svin$dir2, sep=':')
        svs <- inner_join(svs, svin, by='mutation_id')
        svs$sv <- svs$mutation_id; svs$side <- side
        svs <- attach_001_ground_truth_svs(wd_truth, svs)

        svs$true_ccf <- 1
        svs[svs$sample%in%'bM_only','true_ccf'] <- i
        svs[svs$sample%in%'gM_only','true_ccf'] <- 1-i

        svs$adjusted_support <- svs$var_counts
        svs$adjusted_depth <- svs$var_counts + svs$ref_counts

        svs$norm_cn <- svs$normal_cn
        svs$gtype <- ifelse(side == 'side1', svs$gtype1, svs$gtype2)
        x <- t(data.frame(apply(svs, 1, get_true_sc_cn)))
        svs$true_cn <- x[,1]; svs$wtotal_cn <- x[,2]
        svs$pv <- x[,3]; svs$best_ccf <- x[,4]

        svse_data <- rbind(svse_data, svs)
    }
}
...
for(p1 in seq(0.1, 0.9, 0.1)) {
    ...
    x <- sv_data[sv_data$mix%in%mix, ]
    metrics <- rbind(metrics, calc_metrics_3clus(x, mix, 'SVs_dual_end'))

    x <- svse_data[svse_data$mix%in%mix & svse_data$side%in%'side1', ]
    metrics <- rbind(metrics, calc_metrics_3clus(x, mix, 'SVs_single_end1', type='snv'))

    x <- svse_data[svse_data$mix%in%mix & svse_data$side%in%'side2', ]
    metrics <- rbind(metrics, calc_metrics_3clus(x, mix, 'SVs_single_end2', type='snv'))
}
```
...
g2 <- ggplot(mm[mm$variable%like%'cluster CCF',], aes(mix, value, group=method, colour=method)) +
        geom_line() + geom_point() + facet_grid(~variable) + theme_bw() + ylim(-0.6, 0.6) +
...
```{r SuppFigure10, fig.height=7, fig.width=6}
x <- sv_data
x$ccf_mean <- apply(x[,c('ccube_ccf1', 'ccube_ccf2')], 1, mean)
x$is_subclonal_truth <- !x$sample%in%'shared'
...
rocs[['SVs_dual_end']] <- roc(x$is_subclonal_truth, x$ccf_mean)
rocs[['SVs_single_end']] <- roc(y$is_subclonal_truth, y$ccube_ccf)
...
ggrocs(rocs) + theme(axis.text = element_text(size = tsize),
                     ...
                     legend.position = 'bottom') + geom_step(size=1) +
                scale_colour_brewer(palette = colpal)
```
Respective AUCs
...
for(i in seq(0.1, 0.9, 0.1)) {
    ...
    ccubeResultSNVRDataFile = paste0(clonal_basedir, basename, '/ccube_out/snvs/',
                                     basename, '_ccube_snv_results.RData')
...
```{r run_cn_perturbation_experiments}
outdir <-  '001_svclone_results/001_CNdiv/001bM_p0.7_001gM_p0.3/'

het_frac <- 0.5
frac_divergence <- 0.3
magnitude <- 1

p1 <- i <- 0.7; p2 <- 1 - p1; mix <- paste(p1, p2, sep='-')
basename <- paste('001bM_p', p1, '_001gM_p', p2, sep='')

# uncomment below to generate data
# SVclone will then need to be run on output
#
# svin <- read.delim(paste0(basedir, basename, '/', basename, '_filtered_svs.tsv'), sep='\t')
# het_cns <- table(svin$gtype1==svin$gtype2)[1]
# non_het_cns <- as.numeric(round(het_cns/het_frac) - het_cns)
# SEED <- 123; set.seed(SEED); svin <- svin[sample(nrow(svin)),] #shuffle
# svin <- rbind(svin[svin$gtype1==svin$gtype2,][1:non_het_cns,],
#               svin[svin$gtype1!=svin$gtype2,])
# 
# # generate reduced normal set
# system(paste('mkdir -p', outdir))
# system(paste('mkdir -p', paste0(outdir, basename)))
# write.table(svin, file=paste0(outdir, basename, '/', basename, '_filtered_svs.tsv'), quote=F, sep='\t', row.names=F)
# pp_file <- paste0(basedir, basename, '/purity_ploidy.txt')
# system(paste('cp', pp_file, paste0(outdir, basename, '/purity_ploidy.txt')))
# 
# generate_de_cn_experiment(svin, basedir, basename, outdir, magnitude, two_sided=FALSE)
# generate_de_cn_experiment(svin, basedir, basename, outdir, magnitude, two_sided=TRUE)
# generate_de_cn_experiment(svin, basedir, basename, outdir, magnitude * -1, two_sided=FALSE)
# generate_de_cn_experiment(svin, basedir, basename, outdir, magnitude * -1, two_sided=TRUE)
# generate_de_cn_experiment(svin, basedir, basename, outdir, frac_divergence, two_sided=FALSE, frac=TRUE)
# generate_de_cn_experiment(svin, basedir, basename, outdir, frac_divergence, two_sided=TRUE, frac=TRUE)
# 
# generate_se_cn_experiment(svin, basedir, basename, outdir, 0) #normals
# generate_se_cn_experiment(svin, basedir, basename, outdir, frac_divergence, frac=TRUE)
# generate_se_cn_experiment(svin, basedir, basename, outdir, magnitude * -1)
# generate_se_cn_experiment(svin, basedir, basename, outdir, magnitude)
```

```{r load_cn_perturbation_experiments}
svd_data <- NULL
metrics <- NULL

svs <- get_svde_data(basedir, outdir, basename)
svs$experiment <- 'dual-end'; svd_data <- rbind(svs, svd_data)
metrics <- calc_metrics_3clus(svs, mix, 'dual-end', type='sv')
metrics$side <- 'unmodified'
metrics$model <- 'dual'
metrics$type <- 'unmodified'

for(side in c('oneside', 'twoside')) {
    for(type in c('minusCN', 'plusCN', 'frac')) {
        outdir <- paste0('001_svclone_results/001_CNdiv/', paste(basename, side, type, sep='_'))
        svs <- get_svde_data(basedir, outdir, basename)
        
        type_label <- ifelse(type=='minusCN', 'CN - 1', 'CN + 1')
        type_label <- ifelse(type=='frac', 'frac ± 0.3', type_label)
        side_label <- ifelse(side=='oneside', '(one side)', '(both sides)')
        label <- paste(type_label, side_label)
        svs$experiment <- label
        
        metric <- calc_metrics_3clus(svs, mix, label, type='sv')
        metric$side <- side; metric$model <- 'dual'; metric$type <- type_label
        metrics <- rbind(metrics, metric)
        
        svd_data <- rbind(svs, svd_data)
    }
}
...
for(side in c('side1', 'side2')) {
    for(type in c('minusCN', 'plusCN', 'frac', '')) {
        dirname <- ifelse(type!='', paste(basename, side, type, sep='_'), paste(basename, side, sep='_'))
        outdir <- paste0('001_svclone_results/001_CNdiv/', dirname)
        svs <- get_svse_data(basedir, outdir, basename)
        
        type_label <- ifelse(type=='minusCN', 'CN - 1', 'CN + 1')
        type_label <- ifelse(type=='frac', 'frac ± 0.3', type_label)
        type_label <- ifelse(type=='', '', type_label)
        label <- ifelse(type=='', paste('single-end', side), paste(type_label, side))
        svs$experiment <- label
        
        metric <- calc_metrics_3clus(svs, mix, label, type='snv')
        metric$side <- side; metric$model <- 'single'; metric$type <- ifelse(type=='', 'unmodified', type_label)
        metrics <- rbind(metrics, metric)
        
        svs_data <- rbind(svs, svs_data)
    }
}
```
...
```{r SuppFigure11, fig.height=7, fig.width=11}
mm <- melt(metrics)
mm$variable <- as.character(mm$variable)
mm[mm$variable%like%'mean_mult_error',]$variable <- 'mean multiplicity error'
mm[mm$variable%like%'mean_ccf',]$variable <- 'mean variant CCF error'
...
mm[mm$variable%like%'clus_ccf',]$variable <- 'mean cluster CCF error'
mm[mm$variable%like%'clus_num',]$variable <- 'cluster number error'
mm$variable <- factor(mm$variable, levels=c('cluster number error', 'mean variant CCF error',
                                            'is subclonal specificity', 'mean cluster CCF error',
                                            'mean multiplicity error', 'is subclonal sensitivity'))

mm$experiment <- 'CN CCF deviation'
mm[mm$type%like%'CN', 'experiment'] <- 'CN deviation'
mm[mm$type%in%'unmodified', 'experiment'] <- 'unmodified'

mm$side <- 'unmodified'
mm$side[mm$method%like%'one'] <- 'one side'
mm$side[mm$method%like%'two'] <- 'both sides'
mm$side[mm$method%like%'side1'] <- 'side 1'
mm$side[mm$method%like%'side2'] <- 'side 2'

dual <- mm[mm$model%like%'dual',]
dual$method <- factor(dual$method, levels=c('frac ± 0.3 (both sides)', 'frac ± 0.3 (one side)',
                                            'CN - 1 (both sides)', 'CN - 1 (one side)',
                                            'CN + 1 (both sides)', 'CN + 1 (one side)', 'dual-end'))

cols <- brewer.pal(3, 'Dark2')
names(cols) <- c('unmodified', 'CN CCF deviation', 'CN deviation')

ggplot(dual, aes(method, value, group=method, fill=experiment)) +
    geom_bar(stat='identity') + theme_bw() + 
    geom_hline(yintercept=0, colour='grey')+ xlab('') + ylab('') +
    theme(legend.position='bottom', axis.text.x = element_text(size=8), legend.title = element_blank()) +
    scale_fill_manual(values = cols) + facet_wrap(~variable, scales='free_x') +
    coord_flip() + geom_hline(data=dual[dual$method=='dual-end',], aes(yintercept=value), colour='grey')
```
...
```{r SuppFigure12, fig.height=7, fig.width=11}
single <- mm[mm$model%like%'single',]
single <- rbind(single, mm[mm$method=='dual-end',])
single$experiment <- as.character(sapply(as.character(single$method), function(x){strsplit(x, " side")[[1]][1]}))

x <- mm[(mm$model=='dual' & mm$side=='one side') | mm$model=='single' | mm$type=='unmodified',]
x <- data.table(x)[,mean(value),by=c('type', 'variable', 'model')]
x$type <- factor(x$type, levels=c('frac ± 0.3', 'CN + 1', 'CN - 1', 'unmodified'))

ggplot(x, aes(type, V1,  fill=model)) +
    geom_bar(stat='identity', position='dodge') + theme_bw() + 
    geom_hline(yintercept=0, colour='grey')+ xlab('') + ylab('') +
    theme(legend.position='bottom', axis.text.x = element_text(size=8)) +
    scale_fill_brewer(palette = 'Dark2') + facet_wrap(~variable, scales='free_x') +
    coord_flip() + geom_hline(data=x[x$type%like%'unmodified',], aes(yintercept=V1), colour='grey')
```
