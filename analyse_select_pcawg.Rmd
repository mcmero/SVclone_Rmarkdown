...
for (sample in pcawg_summary$sample) {
    ...
    sv_table <- data.frame(sample=sample, nsv=nrow(svcc), nsnv=nrow(snvcc),
                           sv_sc_frac=sv_sc_frac, snv_sc_frac=snv_sc_frac, type=type)
...
```{r Figure5a, fig.height=8, fig.height=7}
...
```{r pcawg_cn_heterogeneity}
pcawg_cn_het <- NULL
for (sample in unique(svsf$sample)) {
    svf_file <- paste(sv_filt_dir, sample, '/', sample, '_filtered_svs.tsv', sep='')
    if(file.exists(svf_file)) {
        svf <- read.delim(svf_file, sep='\t')
        hist <- pcawg_summary$histology_abbreviation[pcawg_summary$sample==sample]
        hetCN <- sum(svf$gtype1 != svf$gtype2, na.rm=TRUE) / nrow(svf)
        pcawg_cn_het <- rbind(pcawg_cn_het, 
                              data.frame(sample=sample, histology=hist,
                                         nsvs=nrow(svf),
                                         heterogenousCN=hetCN))
    }
}
pcawg_cn_het <- pcawg_cn_het[pcawg_cn_het$nsvs > 10,]
pcawg_cn_het <- pcawg_cn_het[pcawg_cn_het$histology%in%svsf$histology_detailed,]

ranks <- data.table(pcawg_cn_het)[, median(heterogenousCN), by=c('histology')]
ranks <- ranks[order(rank(V1))]
pcawg_cn_het$histology <- factor(pcawg_cn_het$histology, levels=ranks$histology)

ggplot(pcawg_cn_het, aes(histology, heterogenousCN)) + geom_boxplot() + 
    geom_jitter(width=0.1, alpha=1, size=0.2) + theme_bw() + theme(axis.text.x=element_text(angle=90))

ggplot(pcawg_cn_het, aes(nsvs, heterogenousCN)) + geom_point(size=0.5) + 
    theme_bw() +  
...
sv_sc_by_type <- rbind(sv_sc_by_type, data.frame(histology='total or mean', 
                                                 ...
                                                 prop_greater_subclonal_svs=mean(sv_sc_by_type$prop_greater_subclonal_svs),
                                                 mean_svs=mean(sv_sc_by_type$mean_svs),
                                                 mean_snvs=mean(sv_sc_by_type$mean_snvs)))
print(sv_sc_by_type)
```
...
```{r Figure5b, fig.height=4, fig.width=6}
...
```{r Figure5d, fig.height=5, fig.width=7}
...
```{r Figure5e, fig.height=5, fig.width=7}
...
```{r Figure5f, fig.height=5, fig.width=7}
plot_hist_func(svcc[svcc$CCF>=subclonal_cutoff,], sc[sc$CCF >= subclonal_cutoff,], 
                 pur, varclass=TRUE, vaf=FALSE, clus = -1,
